...
import { from, map, merge, mergeMap, scan } from 'rxjs';
import {
  DataStreamDocsStat,
  DataStreamStat,
...
import { DataStreamType, TimeRangeConfig } from '../../../../common/types';
...
const getPromiseActorPlaceholder = <Output, Input>() =>
  (() => {
    throw new Error('Not implemented');
  }) as unknown as PromiseActorLogic<Output, Input>;
...
const loadDataStreamStats = ({
  dataStreamStatsClient,
}: {
  dataStreamStatsClient: IDataStreamsStatsClient;
}) =>
  fromPromise<{}, { types: DataStreamType[]; datasetQuery: string }>(
    ({ input: { types, datasetQuery } }) =>
...
const loadDataStreamDocsStats = (dataStreamStatsClient: IDataStreamsStatsClient) =>
  fromObservable<
    DictionaryType<DataStreamDocsStat>,
    { timeRange: TimeRangeConfig; types: DataStreamType[] }
  >(({ input: { timeRange, types } }) => {
    const { startDate: start, endDate: end } = getDateISORange(timeRange);

    return from(types).pipe(
      mergeMap((type) =>
        from(dataStreamStatsClient.getDataStreamsTotalDocs({ type, start, end })).pipe(
          map((data) => ({ type, data }))
        )
      ),
      scan(
        (accumulatedStats, { type, data }) => ({ ...accumulatedStats, [type]: data }),
        {} as DictionaryType<DataStreamDocsStat>
      )
    );
  });

export const pureDatasetQualityControllerStateMachine = setup({
  types: {
    input: {} as DatasetQualityControllerContext,
    context: {} as DatasetQualityControllerContext,
    events: {} as DatasetQualityControllerEvent,
    actors: {
      loadDataStreamStats: getPlaceholderFor(loadDataStreamStats),
      loadDataStreamDocsStats: getPlaceholderFor(loadDataStreamDocsStats),
