...
test('getMergedAccessor works with an empty list of profiles', () => {
  ...
  expect(getTopNavItems(0)).toEqual([{ __brand: 'TopNavItem', name: 'BaseTopNavItem1' }]);
...
test('getMergedAccessor works with a list of profiles', () => {
  const baseProfile: Profile = {
    getTopNavItems: (count: number) => [
      { __brand: 'TopNavItem', name: `BaseTopNavItem1: ${count}` },
    ],
    getDefaultColumns: () => [{ __brand: 'Columns', name: 'BaseColumn1' }],
    getFlyout: () => ({ __brand: 'FlyoutComponent', name: 'BaseFlyoutComponent' }),
  };
  ...
  const profile1: ComposableProfile = {
    getTopNavItems: (prev) => (c: number) =>
      [...prev(c), { __brand: 'TopNavItem', name: 'Profile1TopNavItem1' }],
  };

  const profile2: ComposableProfile = {
    getTopNavItems: (previousGetTopNavItems) => (count: number) =>
      [
        ...previousGetTopNavItems(count - 1),
  ...
  expect(getTopNavItems(2)).toEqual([
